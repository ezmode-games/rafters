...
var Ee = r.forwardRef((e, t) => {
    const {
      ...
      g = q(t, v),
      x = V(i),
      [w, m] = k({ prop: u, defaultProp: p ?? null, onChange: f, caller: A }),
      [I, _] = r.useState(!1),
      T = he(c),
      h = B(a),
      S = r.useRef(!1),
            ...
            if (b.target === b.currentTarget && be && !I) {
              ...
              if ((b.currentTarget.dispatchEvent(L), !L.defaultPrevented)) {
                const M = h().filter((N) => N.focusable),
                  ge = M.find((N) => N.active),
                  Te = M.find((N) => N.id === w),
                  ye = [ge, Te, ...M].filter(Boolean).map((N) => N.ref.current);
                H(ye, s);
              }
            }
            S.current = !1;
  ...
  Y = r.forwardRef((e, t) => {
    const {
      ...
      v = B(a),
      { onFocusableItemAdd: g, onFocusableItemRemove: x, currentTabStopId: w } = s;
    return (
      r.useEffect(() => {
        if (n) return g(), () => x();
      }, [n, g, x]),
...
var J = (e) => {
  const { present: t, children: a } = e,
    n = Oe(t),
    o = typeof a == 'function' ? a({ present: n.isPresent }) : r.Children.only(a),
    i = q(n.ref, De(o));
...
var C = 'Tabs',
  ...
  X = r.forwardRef((e, t) => {
    const {
      ...
      s = V(p),
      [d, v] = k({ prop: n, onChange: o, defaultProp: i ?? '', caller: C });
    return l.jsx(Ue, {
      scope: a,
      baseId: K(),
      value: d,
