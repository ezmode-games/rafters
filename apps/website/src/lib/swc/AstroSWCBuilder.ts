...
import type { ComponentIntelligence, RegistryComponent } from './types';
...
export class AstroSWCBuilder {
  ...
  async buildMultipleComponents(
    ...
    const results = await Promise.allSettled(
      components.map(({ name, props = {} }) => this.executeFullPipeline(name, props))
    );
    ...
    for (let i = 0; i < results.length; i++) {
      ...
      if (result.status === 'fulfilled') {
        ...
        console.error(`Failed to build ${componentName}:`, result.reason);
      }
    }
  ...
  private async executeFullPipeline(
    ...
    try {
      ...
      const fetchResult = await this.fetcher.fetchComponent(componentName);
      ...
      if (!componentFile) {
        throw this.createError(
          componentName,
          'fetch',
          new Error(`No component file found in registry for ${componentName}`)
        );
      }

      // Phase 2: Compile TypeScript/JSX with SWC
      const compileResult = await this.compiler.compile(componentFile.content, {
        filename: componentFile.path,
      });

      // Phase 3: Execute component with React SSR
      const executeResult = await this.executor.execute(compileResult.code, props, {
        componentName,
      });
   ...
   */
  private extractIntelligence(component: RegistryComponent): ComponentIntelligence | null {
   ...
   */
  private determineErrorPhase(error: unknown): 'fetch' | 'compile' | 'execute' {
