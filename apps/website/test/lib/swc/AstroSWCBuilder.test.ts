...
import type { BuildResult } from '../../../src/lib/swc/AstroSWCBuilder';
...
describe('AstroSWCBuilder', () => {
	...
	describe('buildComponentPreview', () => {
		it('should build simple component successfully', async () => {
			...
			vi.mocked(
				SWCCompiler.SWCCompiler.prototype.init,
			).mockResolvedValue();

			vi.mocked(
				SWCCompiler.SWCCompiler.prototype.compile,
			).mockResolvedValue({
			...
			vi.mocked(
				ReactSSRExecutor.ReactSSRExecutor.prototype.execute,
			).mockResolvedValue({
	...
	describe('buildMultipleComponents', () => {
		it('should build multiple components in parallel', async () => {
			...
			vi.mocked(
				ReactSSRExecutor.ReactSSRExecutor.prototype.execute,
			).mockImplementation((code, props, options) =>
				Promise.resolve({
	...
	describe('error handling', () => {
		it('should throw AstroSWCError for fetch phase failures', async () => {
			...
			await expect(
				builder.buildComponentPreview('missing-component'),
		...
		it('should throw AstroSWCError for compile phase failures', async () => {
			...
			vi.mocked(
				SWCCompiler.SWCCompiler.prototype.compile,
			).mockRejectedValue(compilationError);

			await expect(
				builder.buildComponentPreview('button'),
			).rejects.toMatchObject({
				name: 'AstroSWCError',
				componentName: 'button',
				phase: 'compile',
		...
		it('should throw AstroSWCError when component has no files', async () => {
			...
			await expect(
				builder.buildComponentPreview('empty'),
			).rejects.toMatchObject({
				name: 'AstroSWCError',
				componentName: 'empty',
				phase: 'fetch',
