 ...
 */
export function detectImportAlias(cwd = process.cwd()): string | null {
...
export function transformImports(
  ...
  if (alias) {
    ...
    if (componentsDir !== './src/components/ui') {
      ...
      // TODO: Use calculated paths for advanced component directory resolution
      ...
      const relativePath =
        relativeDepth > 2 ? `${'../'.repeat(relativeDepth - 2)}../lib/utils` : '../lib/utils';
  ...
  transformedContent = transformedContent.replace(
    /import.*from\s+['"]@rafters\/design-tokens\/motion['"];?\n?/g,
    ''
  );
