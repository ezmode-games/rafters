 ...
 */
function tokenValueToCss(value, _tokenName) {
 ...
 */
function generateThemeBlock(registry) {
    const lines = ['@theme {'];
    ...
    const tokenMap = Array.from(allTokens).reduce((map, token) => map.set(token.name, token), new Map());
                ...
                if (!isColorFamily && !isScaleToken && !isStateToken && !isNumberedToken) {
                    ...
                    if (typeof token.value === 'string') {
                        ...
                        if (colorValue.value && typeof colorValue.value === 'string') {
                            ...
                            const targetIndex = 5;
                            const oklch = colorValue.scale[targetIndex] ||
 ...
 */
function generateThemeInlineBlock(registry) {
 ...
 */
function generateRootBlock(registry) {
    const lines = [':root {'];
    ...
    for (const token of allTokens) {
        if (token.category === 'color') {
            ...
            if (!rule || (!rule.startsWith('state:') && !rule.startsWith('scale:'))) {
                ...
                if (token.semanticMeaning?.includes('foreground') ||
 ...
 */
function generateDarkModeBlock(registry) {
 ...
 */
function generateContainerQueries(registry) {
    const lines = [
        '/* Container Queries */',
 ...
 */
function generateCustomUtilities() {
    return [
 ...
 */
export function exportToTailwindV4Complete(registry) {
    const sections = [];
