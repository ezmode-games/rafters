 ...
 */
import { findAccessibleColor, generateColorValue, generateOKLCHScale, meetsWCAGStandard, } from '@rafters/color-utils';
/**
 ...
 */
function generateSystemGrayscale(primaryTone) {
    // Use a neutral gray base for system grayscale
    const neutralGray = { l: 0.5, c: 0.02, h: primaryTone.h, alpha: 1 };
 ...
 */
function generateSemanticColorsWithValidation(_primaryColor) {
    ...
    for (const [semantic, color] of Object.entries(result)) {
        ...
        result[semantic] = accessibleColor;
    }
    return result;
}
/**
 ...
 */
function createIntelligentColorValue(baseColor, semanticToken, usage) {
...
export function generateColorTokens() {
    ...
    for (const def of semanticDefinitions) {
        ...
        const token = {
            ...
            consequence: def.trustLevel === 'critical'
                ? 'destructive'
                : def.trustLevel === 'high'
                    ? 'significant'
                    : 'reversible',
        ...
        const foregroundValue = createIntelligentColorValue(foregroundColor, `${def.semantic}-foreground`, `Text color for ${def.semantic} backgrounds`);
