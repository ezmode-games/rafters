 ...
 */
function tokenValueToCss(value, tokenName, registry) {
    ...
    if (value.scale && value.scale.length > 0) {
        ...
        if (targetOklch) {
            const alpha = targetOklch.alpha !== undefined && targetOklch.alpha !== 1 ? ` / ${targetOklch.alpha}` : '';
 ...
 */
export function exportToTailwindCSS(registry) {
    ...
    for (const [category, categoryTokens] of tokensByCategory.entries()) {
        ...
        if (category === 'color') {
            ...
            const scaleTokens = new Map(); // base name -> scale tokens
            const baseTokens = [];
            ...
            for (const token of categoryTokens) {
                ...
                if (rule?.startsWith('scale:')) {
                    ...
                    if (!scaleTokens.has(baseTokenName)) {
            ...
            for (const token of baseTokens) {
                if (processedColors.has(token.name))
                    continue;
            ...
            for (const [baseTokenName, scaleTokenArray] of scaleTokens.entries()) {
                if (processedColors.has(baseTokenName))
                    continue;
                processedColors.add(baseTokenName);
                // Sort scale tokens by scale number
                scaleTokenArray.sort((a, b) => {
                    const aScale = registry.dependencyGraph.getGenerationRule(a.name)?.replace('scale:', '') || '0';
                    const bScale = registry.dependencyGraph.getGenerationRule(b.name)?.replace('scale:', '') || '0';
 ...
 */
export function exportToCSSVariables(registry) {
 ...
 */
export function exportColorScales(registry) {
    const tokens = registry.list();
 ...
 */
export function exportTokensFromRegistry(registry, format = 'tailwind') {
    switch (format) {
        case 'tailwind':
