 ...
 */
function _tokenValueToCssDark(value, stateName = 'default') {
    if (typeof value === 'string') {
        return value;
    }
 ...
 */
function generateCssCustomProperties(designSystem) {
    const lightTokens = [];
    const darkTokens = [];
 ...
 */
function generateTailwindStylesheet(designSystem) {
    ...
    if (tokensByCategory.color) {
        ...
        const normalColorTokens = tokensByCategory.color.filter((token) => !token.name.includes('-dark'));
    ...
    if (tokensByCategory.spacing) {
        ...
        const baseSpacingTokens = tokensByCategory.spacing.filter((token) => !token.name.includes('-') && !token.name.includes('@'));
 ...
 */
export const checkTailwindVersion = async (cwd) => {
    try {
 ...
 */
export const createRegistry = (tokensDir) => {
    const tokens = [];
    try {
 ...
 */
export const createTokenRegistry = (tokensDir) => {
 ...
 */
export const fetchStudioTokens = async (shortcode, tokensDir) => {
 ...
 */
function mapCategoryToTheme(category, tokenName) {
...
export const writeTokenFiles = async (tokenSet, format, cwd) => {
    ...
    const isTestMode = process.env.NODE_ENV === 'test' || process.env.CI === 'true' || process.env.VITEST === 'true';
    if (!isTestMode) {
        ...
        for (const token of colorTokens) {
            try {
                ...
                console.log(`    ⚠ Skipped ${token.name}: ${error instanceof Error ? error.message : 'Unknown error'}`);
            }
        }
        ...
        if (processedColorTokens.length > 0) {
            ...
            const colorData = {
                category: 'color',
                generatedAt: new Date().toISOString(),
                tokens: processedColorTokens.map((token) => ({
                    ...token,
                    // Clean up undefined values for JSON
                    ...Object.fromEntries(Object.entries(token).filter(([_, v]) => v !== undefined)),
                })),
    ...
    console.log(`  ✓ Created ${Object.keys(tokensByCategory).length} category files in .rafters/tokens/`);
 ...
 */
export const injectCSSImport = async (cssFilePath, cwd) => {
 ...
 */
function generateCompleteDesignSystemCSS(tokensContent) {
    return `/**
...
export const exportTokens = (designSystem, format) => {
 ...
 */
export const regenerateCSS = async (cwd = process.cwd()) => {
    const raftersDir = join(cwd, '.rafters');
    const tokensDir = join(raftersDir, 'tokens');
    ...
    let config = { tokenFormat: 'tailwind' };
