 ...
 */
export function generateDesignSystem(primaryColor, config) {
 ...
 */
export function exportToTailwind(system) {
...
${colorTokens.map(token => `  --color-${token.name}: ${token.value};`).join('\n')}
  ...
  /* Typographic Scale */
${Object.entries(system.typography.scale).map(([name, value]) => `  --text-${name}: ${value}rem;`).join('\n')}
...
/*
${system.tokens.filter(token => token.aiIntelligence).map(token => `${token.name}: ${token.aiIntelligence}`).join('\n')}
*/`;
}
/**
