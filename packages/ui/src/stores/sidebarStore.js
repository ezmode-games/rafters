...
export const useSidebarStore = create()(devtools(persist((set, get) => ({
    ...defaultState,
    // Navigation Actions
    navigate: (path, onNavigate) => {
        try {
            ...
            set((state) => ({
                ...
                timestamp: Date.now(),
            }), false, 'sidebar/navigate-fallback');
            onNavigate?.(path);
        }
    },
    setCurrentPath: (path) => {
    ...
    updatePreferences: (preferences) => {
        try {
            ...
            set((state) => ({
                ...
                timestamp: Date.now(),
            }), false, 'sidebar/updatePreferences');
        }
        catch (error) {
    ...
    resetPreferences: () => {
        set((state) => ({
            ...
            timestamp: Date.now(),
        }), false, 'sidebar/resetPreferences');
    },
    ...
    initialize: (initialState) => {
        try {
            ...
            set(() => ({
                ...
                timestamp: Date.now(),
            }), false, 'sidebar/initialize-fallback');
        }
    },
    reset: () => {
        set(() => ({
