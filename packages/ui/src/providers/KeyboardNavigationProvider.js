...
const matchesKeyConfig = (event, config) => {
 ...
 */
export const KeyboardNavigationProvider = ({ children, enableTypeAhead = true, typeAheadDelay = 1000, onGlobalKeyAction, }) => {
    ...
    const unregisterKeyboardHandler = useCallback((menuId) => {
        setState((prev) => {
    ...
    const setGlobalShortcut = useCallback((shortcut, config) => {
        try {
    ...
    const removeGlobalShortcut = useCallback((shortcut) => {
        setState((prev) => {
    ...
    const triggerAction = useCallback((menuId, action, event) => {
    ...
    const enableSearchMode = useCallback((_menuId) => {
        setState((prev) => ({
            ...prev,
            searchMode: true,
            searchTerm: '',
    ...
    const isHandlerEnabled = useCallback((menuId) => {
        const handler = state.handlers.get(menuId);
        return handler?.enabled ?? false;
    }, [state.handlers]);
    // Navigation helpers
    const getNextNavigableItem = useCallback((container, current) => {
    ...
    const getPreviousNavigableItem = useCallback((container, current) => {
        const items = getNavigableElements(container);
        const currentIndex = items.indexOf(current);
        if (currentIndex === -1)
    ...
    const findItemsByText = useCallback((container, searchText) => {
        const items = getNavigableElements(container);
    ...
    return (_jsx(KeyboardNavigationContext.Provider, { value: contextValue, children: children }));
};
/**
 ...
 */
export const useMenuKeyboard = (menuId, menuType, onAction, customKeyConfigs) => {
    ...
    return {
        triggerAction: (action, event) => navigation.triggerAction(menuId, action, event),
...
//# sourceMappingURL=KeyboardNavigationProvider.js.map