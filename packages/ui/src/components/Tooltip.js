 ...
 */
export const Tooltip = forwardRef(function Tooltip({ delayDuration, context = 'help', complexity = 'simple', essential = false, ...props }, _ref) {
    // Timing Intelligence: Smart delays based on context and complexity
    const getIntelligentDelay = () => {
        if (delayDuration !== undefined)
            return delayDuration;
        if (essential)
            return 500; // Essential info - faster access
        if (complexity === 'detailed')
            return 1200; // Complex info - prevent accidental triggers
        if (context === 'shortcut')
            return 300; // Shortcuts - quick access for power users
        if (context === 'status')
            return 600; // Status - balance between info and interruption
        return 700; // Default - balanced for most use cases
    };
    return _jsx(TooltipPrimitive.Root, { delayDuration: getIntelligentDelay(), ...props });
});
/**
 ...
 */
export const TooltipContent = forwardRef(function TooltipContent({ className, sideOffset = 4, context = 'help', complexity = 'simple', essential = false, expandable = false, children, ...props }, ref) {
    return (_jsx(TooltipPrimitive.Content, { ref: ref, sideOffset: sideOffset, className: cn(
        // Base tooltip styling with semantic tokens
        'z-50 overflow-hidden rounded-md px-3 py-1.5 text-sm', 'bg-popover text-popover-foreground shadow-lg', 'border border-border/50', 
        ...
        'transition-all', 'motion-modal', 'easing-accelerate', 
        // Contextual styling based on tooltip type
        {
            ...
            'bg-secondary text-secondary-foreground border-secondary-foreground/30': context === 'status',
            // Shortcut tooltips - emphasized for power users
            'bg-popover text-popover-foreground border-primary/20 font-mono text-xs': context === 'shortcut',
        }, 
        ...
        expandable && ['cursor-pointer', 'hover:shadow-lg', 'transition-shadow', 'motion-hover'], className), ...props, children: children }));
});
/**
 * TooltipTitle - For structured tooltip content
 */
export const TooltipTitle = forwardRef(function TooltipTitle({ className, ...props }, ref) {
    return (_jsx("div", { ref: ref, className: cn('font-medium text-sm mb-1', 'text-foreground', // Higher contrast for titles
        className), ...props }));
});
/**
 * TooltipDescription - For detailed explanations
 */
export const TooltipDescription = forwardRef(function TooltipDescription({ className, ...props }, ref) {
    return (_jsx("div", { ref: ref, className: cn('text-xs leading-relaxed', 'text-muted-foreground', // Lower contrast for descriptions
        className), ...props }));
});
/**
 * TooltipShortcut - For keyboard shortcuts
 */
export const TooltipShortcut = forwardRef(function TooltipShortcut({ className, ...props }, ref) {
    return (_jsx("span", { ref: ref, className: cn('ml-2 inline-flex items-center gap-1', 'text-xs font-mono font-medium', 'text-muted-foreground', 'bg-muted px-1.5 py-0.5 rounded border', className), ...props }));
});
