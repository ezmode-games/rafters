...
export function Label({ className, required, importance = 'standard', context = 'form', validationState = 'default', helpText, semantic = true, children, ref, ...props }) {
    return (_jsxs("div", { className: cn('space-y-1', semantic && 'semantic-label-container'), children: [_jsxs(LabelPrimitive.Root, { ref: ref, className: cn(
                // Base label styling
                'text-sm leading-none text-foreground', 'peer-disabled:cursor-not-allowed peer-disabled:opacity-disabled', 
                ...
                }, className), ...props, children: [children, required && (_jsx("span", { className: cn('ml-1', validationState === 'error' ? 'text-destructive' : 'text-destructive'), "aria-hidden": "true", children: "*" })), importance === 'optional' && !required && (_jsx("span", { className: "ml-1 text-muted-foreground text-xs font-normal", children: "(optional)" }))] }), helpText && (_jsx("p", { className: cn('text-xs text-muted-foreground', {
                    'text-destructive': validationState === 'error',
                    'text-warning': validationState === 'warning',
                    'text-success': validationState === 'success',
                }), role: validationState === 'error' ? 'alert' : 'status', "aria-live": validationState === 'error' ? 'assertive' : 'polite', children: helpText }))] }));
}
