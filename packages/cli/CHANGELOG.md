...
- feat(init): umbrella onboard wire-up -- detection runs before generators, brand-system prompt walks the user through palette decisions, and chosen palettes overlay the default neutral semantic layer (#1501). New `runOnboardWireUp` step in `rafters init` runs before `generateBaseSystem`: detects existing CSS via the orchestrator, surfaces palettes / brand-system status, runs the brand-import prompt from #1506 when interactive, then merges the user-accepted palette tokens onto the default registry so colliding color names (`empire-500` etc.) win over the defaults. New `--accept-detected` agent flag auto-accepts the highest-confidence detection without prompting. Agent mode without the flag, when a brand system is detected, emits a structured `needsDecision` JSON block and exits without writing `.rafters/`. The pre-#1501 fallback (`maybeOnboardExisting` running AFTER generators) is removed -- the early-detection path is now the primary flow. New mapper module (`packages/cli/src/onboard/mapper.ts`) bridges `OnboardResult` + `BrandImportDecision` into registry-ready tokens plus a persisted import config. `RaftersConfig` gains an optional `import` block that round-trips the user's decisions verbatim; `rafters init --reset` re-applies it without re-prompting (deterministic for CI / agent loops).
- feat(import): brand-import interactive prompt for multi-palette CSS (#1401). When the brand-system classifier flags >=2 palettes, `rafters import` now walks the user through three decisions before writing the pending file: which palette is the primary brand colour, whether the others are mutually-exclusive themes or coexisting families, and whether the default neutral semantic layer should still apply on top. New `--assume-brand=<value>` flag accepts `flat` (skip prompt, treat palettes as flat) or `primary:<name>` (pre-select primary, defaults to coexisting mode and keeping default semantics). Agent / non-interactive mode without the flag emits a structured `needsDecision` JSON block describing what would be asked and exits without writing the pending file -- callers re-run with `--assume-brand=...` after deciding. The captured decision lands on `ImportPending.brandDecision` for downstream consumers (--apply, init wire-up, Studio).
- feat(onboard): importer preserves source semantic mappings rather than treating `--primary: var(--empire-500)` as a literal string (#1404). New `resolveReference` helper in `css-parser.ts` walks `var()` chains up to depth 5 with cycle and unresolved-target detection. Generic CSS importer routes any var-referencing variable through the resolver; the produced token's `value` is the leaf string, and the corresponding `PendingToken.sourceReference` records the immediate target var name (e.g. `--empire-500`) so the user sees their intent preserved in the pending file. Cyclic, max-depth, and unresolved chains emit a warning and skip the token rather than landing a broken value. ImportPending schema gains an optional `sourceReference` field on each PendingToken. ImportResult / OnboardResult gain a `references: Record<string, string>` field plumbing the same data.
- feat(onboard): brand-system classifier signals when source CSS encodes multiple complete palettes (#1403). Runs after ramp detection on every importer; when >= 2 palettes are detected, emits a `brandSystem` block on `.rafters/import-pending.json` with the palette names and the semantic slots (primary, accent, background, etc.) found in the same import. Downstream consumers (init, Studio) use this to know they should pause default semantic assignment and prompt the user instead of letting the default neutral layer win. New shared `PendingBrandSystemSchema` in `@rafters/shared`. ImportResult / OnboardResult gain a `brandSystem: BrandSystemAnalysis` field (always populated -- `detected: false` means no brand system, not "absent").
