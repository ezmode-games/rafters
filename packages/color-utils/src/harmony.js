 ...
 */
function generateOptimalGray(paletteColors) {
 ...
 */
export function generateHarmony(baseColor) {
 ...
 */
export function generateRaftersHarmony(baseColor) {
 ...
 */
export function generateSemanticColorSuggestions(baseColor) {
 ...
 */
function generateColorCombinations(colorScale) {
    const combinations = [];
    ...
    for (const [step, targetRatio] of Object.entries(targetContrasts)) {
        ...
        if (Number.parseInt(step, 10) <= 400) {
            ...
            if (stepNum === 50)
                ...
                calculatedL = 0.7; // Still light but usable
        }
        else {
 ...
 */
export function generateOKLCHScale(baseColor) {
 ...
 */
function _generateSemanticColorSystem(baseColor) {
    ...
    for (const [semanticType, colors] of Object.entries(suggestions)) {
        if (colors.length > 0) {
            ...
            semanticSystem[semanticType].scale = colorScale;
            semanticSystem[semanticType].combinations = combinations;
        }
    }
 ...
 */
export function calculateAtmosphericWeight(color) {
    ...
    let atmosphericRole;
    if (distanceWeight < 0.3)
        atmosphericRole = 'background';
    else if (distanceWeight < 0.7)
 ...
 */
function calculateSimultaneousContrast(baseColor, adjacentColors) {
    ...
    const harmonicTension = Math.min(1, (Math.abs(enhancedLightness - avgLightness) +
        Math.abs(enhancedChroma - avgChroma) * 2 +
        Math.min(hueDifference, 360 - hueDifference) / 180) /
        3);
    const contrastRatio = calculateWCAGContrast({ l: avgLightness, c: avgChroma, h: normalizedAvgHue, alpha: 1 }, enhancedColor);
    return {
        enhancedColor,
        contrastRatio,
        harmonicTension,
 ...
 */
export function calculatePerceptualWeight(color) {
 ...
 */
export function generateSemanticColors(baseColor, semanticSuggestions) {
    ...
    for (const [semanticType, colors] of Object.entries(semanticSuggestions)) {
        ...
        const backgroundColors = enhancedColors.filter((c) => c.atmosphericWeight.atmosphericRole === 'background');
        const foregroundColors = enhancedColors.filter((c) => c.atmosphericWeight.atmosphericRole === 'foreground');
        ...
        if (heavyColors.length > 0) {
            recommendations.push(`Heavy ${semanticType} colors work best for critical actions and alerts`);
        }
        if (lightColors.length > 0) {
            recommendations.push(`Light ${semanticType} colors ideal for backgrounds and subtle indicators`);
        }
