...
export function evaluateWithUnits(
  ...
  if (simpleMultiplication) {
    const left = simpleMultiplication[1];
    const rightRaw = simpleMultiplication[2];
    if (left === undefined || rightRaw === undefined) {
      throw new Error(`Invalid multiplication expression: ${expression}`);
    }
  ...
  if (simpleAddition) {
    const left = simpleAddition[1];
    const right = simpleAddition[2];
    if (left === undefined || right === undefined) {
      throw new Error(`Invalid addition expression: ${expression}`);
    }
